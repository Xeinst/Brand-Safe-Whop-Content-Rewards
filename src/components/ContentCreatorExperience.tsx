...
export function ContentCreatorExperience() {
  const [campaigns, setCampaigns] = useState<Campaign[]>([])
  const [submissions, setSubmissions] = useState<CampaignSubmission[]>([])
  const [loading, setLoading] = useState(true)
  const [activeTab, setActiveTab] = useState<'discover' | 'upload' | 'my-content' | 'earnings'>('discover')
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <nav className="flex space-x-8">
            {[
              ...
              { id: 'my-content', label: 'My Content', icon: Eye },
                      ...
                      <div key={submission.id} className="flex items-center justify-between p-3 bg-green-50 rounded-lg">
                        <div>
                          ...
                          <p className="text-sm text-gray-600">
                            Approved {formatDate(submission.reviewedAt || submission.submittedAt)}
                          </p>
                        </div>
                        <div className="text-right">
                          <p className="font-semibold text-green-600">+{submission.rewardEarned} pts</p>
                        </div>
                      </div>
                    ))}
                </div>
